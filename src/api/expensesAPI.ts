...
export async function createExpense(formData: CreateExpenseForm) {
   try {
...
export async function getExpensesByMonth(month: number, year: number) {
   try {
      ...
      const { data } = await api<Expense[]>(
         `/expenses?month=${year}-${monthString}`
      );
      return data;
   } catch (error) {
      if (isAxiosError(error) && error.response) {
...
export async function getExpensesSummary(month: number, year: number) {
   try {
      ...
      const { data } = await api(
         `/expenses/summary?month=${year}-${monthString}`
      );
      return data;
   } catch (error) {
      if (isAxiosError(error) && error.response) {
